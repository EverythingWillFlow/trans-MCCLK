...
GraphConv:
    ...
    图卷积层，整合多个 Aggregator 层。
    支持节点丢弃（node_dropout）和消息丢弃（mess_dropout）以进行正则化。
    构造基于节点嵌入的邻接矩阵，更新节点和用户表示。
...
'''
推荐系统的主类。
初始化了嵌入和图结构，调用 GraphConv 处理图数据。
结合对比损失函数计算用户与物品的嵌入关系，优化推荐效果。
包含多层图卷积（light_gcn），以及三个对比损失函数（calculate_loss、calculate_loss_1、calculate_loss_2）。


核心细节：

    用户和物品嵌入:
        嵌入初始化使用 xavier_uniform_，用户和物品分别有独立的表示。
        嵌入更新通过 GraphConv 完成，包括多跳（n_hops）的消息传递和特征聚合。

    对比学习（Contrastive Learning）:
        使用三种对比学习方法：calculate_loss、calculate_loss_1 和 calculate_loss_2。
        利用嵌入的相似性构造正样本和负样本对，计算交叉熵损失。
        通过 fc1、fc2 和 fc3 非线性投影学习不同空间的特征对比。

    轻量级图卷积网络（light_gcn）:
        实现了一个轻量级 GCN（LightGCN），仅基于邻接矩阵传播嵌入信息，不使用非线性激活。
        多层嵌入结果的均值作为最终表示。

    BPR 损失函数:
        采用了基于对比学习增强的贝叶斯个性化排序（BPR）损失。
        包括嵌入正则化项和对比损失项，用于优化推荐性能。
